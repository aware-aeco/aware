...
Verified bundles additionally carry `agent-bundle-pins`, so changed bytes require recompilation. `app run --require-verified-agents` snapshots every dispatchable agent once before trace creation and refuses with `E_APP_AGENT_BUNDLE_UNVERIFIED` unless its official registry receipt still matches its tree. Frozen subtrees and `--simulate` dispatch nothing and are excluded; dry-run remains gated because read nodes still execute. The snapshot is recorded as `verified-at-start` in run-start provenance. An app-backed synthesized agent is not implicitly trusted, preventing nested dispatch from bypassing the flag.
