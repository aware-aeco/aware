...
The `app` id is a **plain name**, not a path. It becomes a directory (`~/.aware/apps/<id>/`) and a file name (`<source-dir>/<id>.lock`), so it may not be `.` or `..`, contain a path separator, or carry a drive/UNC prefix. A dot *inside* a name (`my.app`) is fine. `E_APP_ID_NOT_A_SEGMENT` at install, compile and validate. Before that check existed, an id like `../../pwned` compiled clean and wrote its lockfile two directories above the source, reporting success (#365).
...
- the output is **schema-bound** (a fixed JSON schema, not free text a node `eval`s); it **cannot** be an `assert:` evaluator (validators stay LLM-free) and **cannot** branch control flow. "Schema-bound" means **enforced, not requested**: the reply is validated against the declared schema before it is returned, a violation is retried once, and a second violation fails with the offending instance path rather than being handed back as a success. A reply that does not validate is **never cached**, and a cached entry that no longer validates is evicted and re-extracted rather than replayed — otherwise one bad extraction would be permanent for that input. (Embedding the schema in the prompt is how the model is *asked*; validation is what makes it a fence.)
...
The substrate is extension-agnostic: `.app` (the recommended default), `.flow`, `.aware`, or any other extension all compile to the same lockfile shape. The sidecar is named by the app's id, not the source extension.
...
```yaml
# tender-issue.lock — compiled from tender-issue.app
...
| `aware app validate <app>` | Schema + cycle + cap checks. Also emits `<app>.lock` next to the source, and says so — the sidecar is announced, never written silently. The verdict is a fact about the file, so a sidecar that cannot be written (read-only directory, `.lock` name already held by a directory) is reported as a warning and leaves a valid app valid; a *compile* failure is a verdict and still fails the command. |
...
Values, object keys, and elements a `for-each:` drew straight from `secrets` all go. Keys the node's own template names are kept, so `{{ secrets.teams.coord }}` previews as `[redacted]` rather than disappearing — a field that renders empty reads as a *missing* credential, which is worse than one that says it is hidden. The boundary is the vault and one hop from it: a credential a node **read** — out of a file, or computed by a `compare` — is that node's own output, which the trace records in full, and which the runtime cannot tell from any other value.
...
```
welded-to-tc/
├── welded-to-tc.app                # the composition
├── manifest.yaml                   # if needed (extends the app file)
└── skills/
...
An **unreadable** nested pin is the one part of that check that does not wait for dispatch: the caller's pre-flight reads the `requires:` syntax of every installed app-backed agent it can dispatch to, in every run mode. Dispatch is too late under `--simulate`, which stubs the node and never loads the backing app at all — and readability is a fact about a file rather than about the environment, so the simulation exemption does not cover it one level down any more than it does at the top level. That pre-flight reads the nested app's manifest and nothing else: it does not run the nested app, and anything **absent** — an agent that is not installed, a backing app with no `.flo`/`.app` file — is passed over in silence, so `--simulate` remains the way to check a composition before the apps and agents around it are installed. A file that is **present and unreadable** is a fault rather than an absence, and is reported: neither the agent manifest nor the backing app is skipped on a parse or read failure, because a check that cannot read the files it must follow cannot report "no unreadable pin below them" any more than one that cannot read its own constraint can report "satisfied".
