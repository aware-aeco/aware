---
name: tekla-tekla-structures-model-collaboration
description: This skill encodes the tekla 2026.0 surface of the Tekla.Structures.Model.Collaboration namespace — 16 types with full vendor-documented methods, properties, events, and behavioral remarks. Read when composing apps that touch any of: IFC2X3_ParametricObject_CircleHollowProfile, IFC2X3_ParametricObject_CircleProfile, IFC2X3_ParametricObject_CShapeProfile, IFC2X3_ParametricObject_LShapeProfile, IFC2X3_ParametricObject_EllipseProfile, IFC2X3_ParametricObject_IShapeProfile, IFC2X3_ParametricObject_RectangleHollowProfile, IFC2X3_ParametricObject_TShapeProfile, and 8 more types.
---

# Tekla.Structures.Model.Collaboration

Auto-generated from vendor docs for tekla 2026.0. 16 types in this namespace.

## IFC2X3_ParametricObject_CShapeProfile (class)

The IFC2X3_ParametricObject_CShapeProfile class contains the C-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/89328895-cdcd-95bd-1b3a-5dd3baf1b94e)

### Properties
- `Depth` (object, get/set) — The depth.
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Girth` (object, get/set) — The girth.
- `InternalFilletRadius` (object, get/set) — The internal fillet radius.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WallThickness` (object, get/set) — The wall thickness.
- `Width` (object, get/set) — The width.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_CircleHollowProfile (class)

The IFC2X3_ParametricObject_CircleHollowProfile class contains the circular and hollow profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/65098be9-c2c8-961d-287b-33c72e8d7285)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `Radius` (object, get/set) — The radius.
- `WallThickness` (object, get/set) — The wall thickness.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_CircleProfile (class)

The IFC2X3_ParametricObject_CircleProfile class contains the circular profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/0d72d508-d685-beaa-ba69-deb68cf62feb)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `Radius` (object, get/set) — The radius.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_EllipseProfile (class)

The IFC2X3_ParametricObject_EllipseProfile class contains the ellipse profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/3360c59e-eed0-7151-07c3-3a6c94daff6f)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `SemiAxis1` (object, get/set) — The first semi-axis.
- `SemiAxis2` (object, get/set) — The second semi-axis.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_IShapeProfile (class)

The IFC2X3_ParametricObject_IShapeProfile class contains the I-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/34a8aa36-d51f-7f6a-d3d9-2a815df75f8d)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `FilletRadius` (object, get/set) — The fillet radius.
- `FlangeThickness` (object, get/set) — The flange thickness.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `OverallDepth` (object, get/set) — The overall depth.
- `OverallWidth` (object, get/set) — The overall width.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WebThickness` (object, get/set) — The web thickness.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_LShapeProfile (class)

The IFC2X3_ParametricObject_LShapeProfile class contains the L-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/f57e5aa4-f27d-ddef-2280-1bdbc2a4ca34)

### Properties
- `Depth` (object, get/set) — The depth.
- `Description` (object, get/set) — The description of the product.
- `EdgeRadius` (object, get/set) — The edge radius.
- `Extrusion` (object, get/set) — The extrusion vector.
- `FilletRadius` (object, get/set) — The fillet radius.
- `LegSlope` (object, get/set) — The leg slope.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `Thickness` (object, get/set) — The thickness.
- `Width` (object, get/set) — The width.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_RectangleHollowProfile (class)

The IFC2X3_ParametricObject_RectangleHollowProfile class contains the rectangular and hollow profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/7fa6acbf-6174-e4c7-1a73-d26f3e507f6a)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `InnerFilletRadius` (object, get/set) — The inner fillet radius.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `OuterFilletRadius` (object, get/set) — The outer fillet radius.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WallThickness` (object, get/set) — The wall thickness.
- `XDim` (object, get/set) — The X dimension.
- `xDir` (object, get/set) — The X-axis direction.
- `YDim` (object, get/set) — The Y dimension.

## IFC2X3_ParametricObject_RectangleProfile (class)

The IFC2X3_ParametricObject_RectangleProfile class contains the rectangular profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/52884a46-3932-c96c-587a-1ff1af047bb2)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `XDim` (object, get/set) — The X dimension.
- `xDir` (object, get/set) — The X-axis direction.
- `YDim` (object, get/set) — The Y dimension.

## IFC2X3_ParametricObject_TShapeProfile (class)

The IFC2X3_ParametricObject_TShapeProfile class contains the T-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/0cfd51a2-981e-508f-59d7-ff84ef6fe101)

### Properties
- `Depth` (object, get/set) — The depth.
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `FilletRadius` (object, get/set) — The fillet radius.
- `FlangeEdgeRadius` (object, get/set) — The flange edge radius.
- `FlangeSlope` (object, get/set) — The flange slope.
- `FlangeThickness` (object, get/set) — The flange thickness.
- `FlangeWidth` (object, get/set) — The flange width.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WebEdgeRadius` (object, get/set) — The web edge radius.
- `WebSlope` (object, get/set) — The web slope.
- `WebThickness` (object, get/set) — The web thickness.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_UShapeProfile (class)

The IFC2X3_ParametricObject_UShapeProfile class contains the U-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/0e00b2e9-7eff-98ba-5db8-721bff48a418)

### Properties
- `Depth` (object, get/set) — The depth.
- `Description` (object, get/set) — The description of the product.
- `EdgeRadius` (object, get/set) — The edge radius.
- `Extrusion` (object, get/set) — The extrusion vector.
- `FilletRadius` (object, get/set) — The fillet radius.
- `FlangeSlope` (object, get/set) — The flange slope.
- `FlangeThickness` (object, get/set) — The flange thickness.
- `FlangeWidth` (object, get/set) — The flange width.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WebThickness` (object, get/set) — The web thickness.
- `xDir` (object, get/set) — The X-axis direction.

## IFC2X3_ParametricObject_ZShapeProfile (class)

The IFC2X3_ParametricObject_ZShapeProfile class contains the Z-shape profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/70245cf7-9d2d-17df-d6cc-260857dcb3ae)

### Properties
- `Depth` (object, get/set) — The depth.
- `Description` (object, get/set) — The description of the product.
- `EdgeRadius` (object, get/set) — The edge radius.
- `Extrusion` (object, get/set) — The extrusion vector.
- `FilletRadius` (object, get/set) — The fillet radius.
- `FlangeThickness` (object, get/set) — The flange thickness.
- `FlangeWidth` (object, get/set) — The flange width.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `WebThickness` (object, get/set) — The web thickness.
- `xDir` (object, get/set) — The X-axis direction.

## ParametricObject_CustomProfile (class)

The ParametricObject_CustomProfile class contains the custom profile definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/8722c3a4-033d-e29b-fecd-46198417bc90)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `xDir` (object, get/set) — The X-axis direction.

## ParametricObject_ObjectBoundingBox (class)

The ParametricObject_ObjectBoundingBox class contains the object bounding box (OBB) definition.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/00bed549-a91f-aefe-d8cf-b1a52ea4d921)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `xDir` (object, get/set) — The X-axis direction.
- `yDir` (object, get/set) — The Y direction.

## ReferenceModelObjectAttribute (class)

The ReferenceModelObjectAttribute class is a base class from where concrete attributes are derived from. The documentation about different attributes and their function can be found from the vendor specification, for example from the IFC format specification.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/61d107a8-97ad-77de-6b59-1e70775ab740)

### Properties
- `Description` (object, get/set) — The description of the product.
- `Extrusion` (object, get/set) — The extrusion vector.
- `Name` (object, get/set) — The name of the product.
- `ObjectType` (object, get/set) — The object type of the product.
- `Origin` (object, get/set) — The origin of the extrusion.
- `ProfileName` (object, get/set) — The name of the profile (often empty).
- `xDir` (object, get/set) — The X-axis direction.

## ReferenceModelObjectAttribute.AttributeTypeEnum (enum)

All the reference model object attribute types.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/991eba0d-eea8-87a6-6b61-ce8a7f07c6f9)

### Values
- `ATTRIBUTETYPE_INVALID` = `0` — The invalid attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_CSHAPEPROFILE` = `3` — The C-shape profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_CIRCLEHOLLOWPROFILE` = `4` — The circular and hollow profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_CIRCLEPROFILE` = `5` — The circular profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_ELLIPSEPROFILE` = `6` — The ellipse profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_ISHAPEPROFILE` = `7` — The I-shape profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_LSHAPEPROFILE` = `8` — The L-shape profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_RECTANGLEHOLLOWPROFILE` = `9` — The rectangular and hollow profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_RECTANGLEPROFILE` = `10` — The rectangular profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_TSHAPEPROFILE` = `11` — The T-shape profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_USHAPEPROFILE` = `12` — The U-shape profile attribute type.
- `ATTRIBUTETYPE_IFC2X3_PARAMETRICOBJECT_ZSHAPEPROFILE` = `13` — The Z-shape profile attribute type.
- `ATTRIBUTETYPE_PARAMETRICOBJECT_CUSTOMPROFILE` = `14` — The custom profile attribute type.
- `ATTRIBUTETYPE_PARAMETRICOBJECT_OBB` = `29` — The object bounding box (OBB) attribute type.

## ReferenceModelObjectAttributeEnumerator (class)

The ReferenceModelObjectAttributeEnumerator class can be used to enumerate through reference model object attributes.

[Vendor docs](https://developer.tekla.com/topic/en/18/47/512af538-e85a-9411-176d-9ce3f55bd4f8)

### Constructors
- `ReferenceModelObjectAttributeEnumerator(...)` — Creates a new reference model object attribute enumerator instance.

### Methods
#### `MoveNext(...)`

Moves to the next reference model object attribute.

[Docs](https://developer.tekla.com/topic/en/18/47/d97e9744-200e-a87a-93ce-38b177e5a4cd)

#### `Reset(...)`

Resets the enumerator.

[Docs](https://developer.tekla.com/topic/en/18/47/8372f374-6d18-d371-d1a7-ddd9e33c4b91)

### Properties
- `Current` (object, get/set) — The current attribute.

