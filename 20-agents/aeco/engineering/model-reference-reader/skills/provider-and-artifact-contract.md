...
reader v2 raises the default source ceiling from 150 MiB to 256 MiB, metadata from 16 MiB to 32 MiB,
canonical GLB JSON from 16 MiB to 32 MiB, indices from 15 million to 20 million, component JSON from
32 MiB to 128 MiB, and canonicalization work from 4 GiB to 8 GiB. These are signed defaults, and
callers may lower them explicitly. A managed protocol-v2 conversion also requires both this exact
reader-schema value and an installer-owned durable UUID `conversion-attempt-id`; callers written for
reader 0.6.0 must supply both when upgrading.

The reader-v1 source admission ceiling remains 150 MiB. Reader-v2's ordinary source admission ceiling
is 256 MiB so its default profile admits at least twice the pinned Snowdon Towers sample. These versioned
defaults are part of the signed canonical request: upgrading changes the request hash, so the first read
after upgrade converts again instead of reusing an entry produced under different limits. The bounded
cache's normal quota maintenance later reclaims unmatched entries by deterministic least-recently-used
eviction; it does not treat an old request hash as current evidence.
