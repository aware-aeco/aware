...
> **Decode it as UTF-8 explicitly.** The output carries non-ASCII characters — `—`, `²`, §, `≤`, `→` and `×` — from the
> Eurocode clause references and symbols. A caller that lets its runtime pick the console codepage instead will **throw while
...
>
> Measured across all 68 rule ids: **3** fail to decode under cp1250 while every one is
