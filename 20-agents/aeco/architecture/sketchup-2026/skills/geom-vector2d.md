...
- `cross` — The #cross method returns the cross product between two Geom::Vector2ds.
...
- `normalize!` — The #normalize! method converts a Geom::Vector2d vector into a unit vector. Another way to do this is vector.length = 1.0
- `parallel?` — The #parallel? method determines if two Geom::Vector2ds are parallel within a tolerance. Two vectors are parallel if there exists a scalar multiple between them.
- `perpendicular?` — The #perpendicular? method determines if two Geom::Vector2ds are perpendicular within a tolerance. Two vectors are considered perpendicular if their dot product is zero.
