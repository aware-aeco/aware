...
> **Major caveat — `conversations.history` / `conversations.replies` are throttled for non-Marketplace apps.** Apps distributed **outside the Slack Marketplace** ("unlisted") are capped at **1 request/minute and 15 objects per page** on these two methods — *not* the Tier 3 budget above — to curb bulk conversation-data exfiltration. Effective dates: new apps + new installs since **2025-05-29**; **existing installs since 2026-03-03** — so as of now this applies to essentially *all* non-Marketplace distributed apps. **Exempt (keep normal limits):** Marketplace-approved apps and **internal customer-built apps**. A self-registered AWARE Slack app used inside your own workspace is internal → fine; a distributed non-Marketplace app is throttled — design history-paging for 1/min + 15/page. (Slack has revised this timeline more than once; verify against the current rate-limits doc.)
