...
var singleAnglePlans = new Dictionary<string,AwareTekla.TeklaSingleAnglePlan>(StringComparer.Ordinal);
...
try {
    ...
    foreach(var pair in elementById){string id=pair.Key;activeId=id;var el=pair.Value;string kind=str(el,"kind").ToLowerInvariant();if(String.IsNullOrEmpty(kind))kind="member";if(realizedChildren.ContainsKey(id))continue;
        if(kind=="member"||kind=="line"||kind=="box"){var from=list(el,"from");var to=list(el,"to");if(!vec3ok(from)||!vec3ok(to))throw new Exception(id+": member requires finite from/to");string profile=str(el,"profile");var em=obj(el,"meta");if(String.IsNullOrWhiteSpace(profile)&&em!=null)profile=str(em,"profile");if(String.IsNullOrWhiteSpace(profile))throw new Exception(id+": member requires an exact requested profile; fallback profiles are forbidden");AwareTekla.TeklaDoubleAnglePlan anglePlan;if(doubleAnglePlans.TryGetValue(id,out anglePlan)){var rolledX=new Vector(memberRollPlans[id].RolledX[0],memberRollPlans[id].RolledX[1],memberRollPlans[id].RolledX[2]);var head=point(from);var tail=point(to);var legGuids=new List<string>();var placedLegs=new List<Beam>();Beam firstLeg=null;foreach(var leg in anglePlan.Legs){var legHead=move(head,rolledX,leg.OffsetMm);var legTail=move(tail,rolledX,leg.OffsetMm);var placed=insertBeam(id,id+"#"+leg.Suffix,"member",anglePlan.LegProfile,leg.ReversedAxis?legTail:legHead,leg.ReversedAxis?legHead:legTail,el);legGuids.Add(placed.Identifier.GUID.ToString());placedLegs.Add(placed);if(firstLeg==null)firstLeg=placed;}var rolledY=new Vector(memberRollPlans[id].RolledY[0],memberRollPlans[id].RolledY[1],memberRollPlans[id].RolledY[2]);var sectionSeen=new List<double[]>();foreach(var legBeam in placedLegs)foreach(var v in solidVertices(legBeam)){var d3=new Vector(v.X-head.X,v.Y-head.Y,v.Z-head.Z);var xy=new[]{dot(d3,rolledX),dot(d3,rolledY)};if(!sectionSeen.Any(q=>Math.Abs(q[0]-xy[0])<=0.1&&Math.Abs(q[1]-xy[1])<=0.1))sectionSeen.Add(xy);}var wanted=new List<double[]>();foreach(var w in anglePlan.SectionOutline)if(!wanted.Any(q=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1))wanted.Add(w);if(sectionSeen.Count!=wanted.Count||wanted.Any(w=>!sectionSeen.Any(q=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1))||sectionSeen.Any(q=>!wanted.Any(w=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1)))throw new Exception(id+": the inserted double-angle pair does not match the canonical `"+str(obj(el,"xsection"),"orientation")+"` section; this environment's `"+anglePlan.LegProfile+"` is seated differently from the one the contract was probed against");doubleAngleLegGuids[id]=legGuids;nativeById[id]=firstLeg;parts[id]=firstLeg;profileById[id]=profile;}else if(singleAnglePlans.ContainsKey(id)){var singlePlan=singleAnglePlans[id];var basePlan=memberRollPlans[id];var head=point(from);var tail=point(to);var placed=insertBeam(id,id+"#L","member",profile,singlePlan.ReversedAxis?tail:head,singlePlan.ReversedAxis?head:tail,el);var rolledX=new Vector(basePlan.RolledX[0],basePlan.RolledX[1],basePlan.RolledX[2]);var rolledY=new Vector(basePlan.RolledY[0],basePlan.RolledY[1],basePlan.RolledY[2]);var sectionSeen=new List<double[]>();foreach(var v in solidVertices(placed)){var d3=new Vector(v.X-head.X,v.Y-head.Y,v.Z-head.Z);var xy=new[]{dot(d3,rolledX),dot(d3,rolledY)};if(!sectionSeen.Any(q=>Math.Abs(q[0]-xy[0])<=0.1&&Math.Abs(q[1]-xy[1])<=0.1))sectionSeen.Add(xy);}string resolvedAngle=profileById[id];if(!AwareTekla.TeklaSingleAngleContract.HeelIsOnTheCanonicalSide(sectionSeen,0.1))throw new Exception(id+": the inserted `"+resolvedAngle+"` seats its heel on the mirror of the canonical `angle`; this environment seats that profile differently from the one the contract was probed against");var measuredAngle=AwareTekla.TeklaSingleAngleContract.MeasureEnvelope(sectionSeen);if(Math.Abs(measuredAngle[0]-singlePlan.EnvelopeWidthMm)>AwareTekla.TeklaSingleAngleContract.EnvelopeToleranceMm||Math.Abs(measuredAngle[1]-singlePlan.EnvelopeDepthMm)>AwareTekla.TeklaSingleAngleContract.EnvelopeToleranceMm)throw new Exception(id+": the inserted `"+resolvedAngle+"` measures "+mm(measuredAngle[0])+"x"+mm(measuredAngle[1])+" where the canonical `angle` descriptor requires "+mm(singlePlan.EnvelopeWidthMm)+"x"+mm(singlePlan.EnvelopeDepthMm));if(String.Equals(resolvedAngle,singlePlan.ParametricProfile,StringComparison.OrdinalIgnoreCase)){var wantedAngle=new List<double[]>();foreach(var w in singlePlan.SectionOutline)if(!wantedAngle.Any(q=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1))wantedAngle.Add(w);if(sectionSeen.Count!=wantedAngle.Count||wantedAngle.Any(w=>!sectionSeen.Any(q=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1))||sectionSeen.Any(q=>!wantedAngle.Any(w=>Math.Abs(q[0]-w[0])<=0.1&&Math.Abs(q[1]-w[1])<=0.1)))throw new Exception(id+": the inserted `"+resolvedAngle+"` does not match the canonical `angle` section; this environment's parametric `L` is seated differently from the one the contract was probed against");}if(!nativeRotationById.ContainsKey(id+"#L")||!nativeRotationOffsetById.ContainsKey(id+"#L"))throw new Exception(id+": single-angle bookkeeping is inconsistent after insert; the mirrored part is in the model but cannot be reported");memberRollById[id]=basePlan.NormalizedDegrees;nativeRotationById[id]=nativeRotationById[id+"#L"];nativeRotationOffsetById[id]=nativeRotationOffsetById[id+"#L"];}else insertBeam(id,id,"member",profile,point(from),point(to),el);}
