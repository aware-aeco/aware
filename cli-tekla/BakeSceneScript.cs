...
try {
    ...
    foreach(var item in supportedOrder){string id=item.Item1;string kind=item.Item2;ModelObject o=null;string realizedBy="";if(nativeById.TryGetValue(id,out o)){}else if(realizedChildren.TryGetValue(id,out realizedBy)||realizedEffects.TryGetValue(id,out realizedBy)||realizedReferences.TryGetValue(id,out realizedBy))nativeById.TryGetValue(realizedBy,out o);if(o==null)throw new Exception(id+": supported record was not classified by the materializer");var r=row(id,kind,"emitted","","");r["nativeGuid"]=o.Identifier.GUID.ToString();if(!String.IsNullOrEmpty(realizedBy))r["realizedBy"]=realizedBy;if(profileById.ContainsKey(id))r["profile"]=profileById[id];if(memberRollById.ContainsKey(id)){r["rot"]=memberRollById[id];r["nativeRotation"]=nativeRotationById[id];r["nativeRotationOffset"]=nativeRotationOffsetById[id];}AwareTekla.TeklaDoubleAnglePlan emittedAnglePlan;if(doubleAnglePlans.TryGetValue(id,out emittedAnglePlan)){var legGuids=doubleAngleLegGuids[id];r["rot"]=memberRollPlans[id].NormalizedDegrees;r["nativeGuids"]=legGuids;r["legs"]=emittedAnglePlan.Legs.Select((leg,index)=>(object)new Dictionary<string,object>(StringComparer.Ordinal){{"suffix",leg.Suffix},{"nativeGuid",legGuids[index]},{"reversedAxis",leg.ReversedAxis},{"offsetMm",leg.OffsetMm},{"nativeRotation",nativeRotationById[id+"#"+leg.Suffix]},{"nativeRotationOffset",nativeRotationOffsetById[id+"#"+leg.Suffix]}}).ToList();gridWarningJournal.Queue(row(id,kind,"warning","tekla-double-angle-materialized-as-pair","Tekla has no native double-angle profile, so requested `"+str(elementById[id],"profile")+"` was materialized as two `"+emittedAnglePlan.LegProfile+"` parts "+String.Join(" and ",legGuids.ToArray())+" separated by the authored gap."));}emitted.Add(r);}
