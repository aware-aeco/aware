...
| [Monday Model Audit](./bim-monday-audit.app) | **BIM manager** (killer app) | DAG (6 nodes) | v0.19 `for-each` + v0.19 `schedule:` + v0.20 atom references *(does not run — see below)* + v0.11 `safety:` + v0.12 Teams card + Outlook attachment |
...
| [Cross-project Sheet+RFI Ball-in-Court](./architect-sheet-status.app) | **Architect** (killer app) | DAG (6 nodes) | v0.19 `schedule:` + `for-each` + v0.14 acc-issues + v0.15 bluebeam-studio + v0.20 atom references *(does not run — see below)*. The "$400/month subscription to delete a 4hr/week task" app. |
...
> **Two of these apps do not run today.** `bim-monday-audit` and
> `architect-sheet-status` each filter with an inline predicate written as an
> `atom://` reference. That URI shape is published in
> [`app-spec.md` § Atom references](../../10-core/app-spec.md#atom-references-v020),
> but no resolver exists in the CLI, so the predicate has no executable body:
> `aware app validate`, `aware app compile` and `aware app run` all refuse it
> with `E_APP_INLINE_NO_BODY`.
>
> That refusal is the fix, not the bug. Until it landed, a body-less predicate
> defaulted to the literal `true`: the gate forwarded its input unfiltered and
> reported `{"pass": true}`, so the Monday audit called every sheet "changed in
> the last 7 days" and the ball-in-court card named every open RFI as aging —
> with nothing in the run log to say the filter had stopped filtering. See #554.
>
> Read those two as reference **topology** until `atom://` resolution ships. The
> other five apps in the table above are unaffected by *this* defect — though
> note that `detailer-issue-pack` and `qa-drawings-to-tekla` compose tekla verbs
> still marked `status: planned`, so they are refused too once a tekla agent is
> installed (see `cli/tests/tekla_verb_parity.rs`).


`aware app install` takes the DIRECTORY holding one app manifest, not the
`.app` file itself:

```bash
# Install (bim-monday-audit and architect-sheet-status are refused — see above)
aware app install ./some-dir-holding-one-app
