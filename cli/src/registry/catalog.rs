...
/// The transport `aware agent catalog` publishes for an agent — which must be the
/// one it would actually RUN on, not a second opinion.
///
...
///
/// `mcp` survives as a fallback rather than a rank: it is reported only when it is
/// all there is, which `validate_agent` now refuses (`E_AGENT_MCP_ONLY`), so this
/// arm describes a manifest that got in without validation. Saying `mcp` there is
/// truthful — it is what the manifest declares — where saying `none` would hide it.
