...
`aware app artifact <app> --instance <instance> --run-id <run> --prune --reservation-id <reservation>` accepts no artifact id, output, byte limit, or usage flag. It prints one JSON object `{app,instance,runId,pruned:true,bytes,files}`. The empty artifact directory remains as the idempotency tombstone; a missing or replaced directory is refused. The provenance trace, reservation marker, and run lease remain for audit and replay. Other artifact operations remain unchanged.

## Approach

1. Create a run-specific kernel lease at `<logs>/<app>/<instance>/<run>.artifact-lease` with exclusive file creation. Its durable JSON names the exact app, instance, run and reservation, plus `writerClass`. The writer holds a **shared** lock from before reservation publication through all dispatch/cleanup; artifact copy/usage also take a shared lock; prune requires an exclusive nonblocking lock. Shared readers therefore remain possible during progressive output. New reservation markers record the lease's and artifact directory's operating-system file identities (Unix device+inode; Windows volume+file index via Win32). Prune compares the locked lease and opened directory handles' identities to this marker, so neither path can be substituted to authorize removal of unrelated files. If a platform cannot supply stable identities, pruning refuses.
...
3. Acquire/construct the lease, exclusively create and sync the empty artifact directory, then publish the extended immutable reservation marker containing the writer class and lease identity. Only then exclusively create the trace. Refuse any pre-existing artifact or trace target for a new run. A crash before marker publication leaves no owned artifact payload; a crash after publication leaves a matching lease and an already-owned artifact directory, even when no trace was created.
