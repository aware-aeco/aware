# regen-rhino.ps1 — re-generate Rhino agent from IR + run strict 50-type verify.
#
# Mirrors the Tekla regen-and-verify.ps1 but for Rhino: vendor `mcneel`, vertical `architecture`.
# Copies manifest.yaml + skills/ + catalog/ into 20-agents/aeco/architecture/rhino-<v>/.
#
# Usage: ./regen-rhino.ps1 <version> [<count>=50]   # version is "7.0" or "8.0"
...
$Ir = Join-Path $RepoRoot "cli-sidecar\Ingest\Output\rhino-$Version.ir.json"
...
& $Aware coverage generate rhino $Version --from-ir $Ir --vendor mcneel --vertical architecture
if ($LASTEXITCODE -ne 0) { Write-Error "coverage generate failed with exit $LASTEXITCODE"; exit 1 }

Write-Host ""
Write-Host "==> Step 2: copy artefacts into 20-agents/aeco/architecture/rhino-$VMajor/"
$src = "$HOME\.aware\agents\rhino-$Version"
$dst = Join-Path $RepoRoot "20-agents\aeco\architecture\rhino-$VMajor"
