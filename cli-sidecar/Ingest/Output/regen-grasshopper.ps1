# regen-grasshopper.ps1 — re-generate Grasshopper agent from IR + run strict 50-type verify.
...
$Ir = Join-Path $RepoRoot "cli-sidecar\Ingest\Output\grasshopper-$Version.ir.json"
...
& $Aware coverage generate grasshopper $Version --from-ir $Ir --vendor mcneel --vertical architecture
if ($LASTEXITCODE -ne 0) { Write-Error "coverage generate failed with exit $LASTEXITCODE"; exit 1 }

Write-Host ""
Write-Host "==> Step 2: copy artefacts into 20-agents/aeco/architecture/grasshopper-$VMajor/"
$src = "$HOME\.aware\agents\grasshopper-$Version"
$dst = Join-Path $RepoRoot "20-agents\aeco\architecture\grasshopper-$VMajor"
