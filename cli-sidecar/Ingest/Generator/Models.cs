...
// policy is applied during deserialization — see IrJsonContext).
//
...
// preserves the IR field name (`namespace`, `params`, …) for the source-
// generated JSON serializer (IrJsonContext).
//
// EnumValueInfo.value is `JsonElement` rather than `object` so the source
...
public record EnumValueInfo(string name, JsonElement value, string? summary);
