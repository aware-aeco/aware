...
$ npm install -g @aware-aeco/cli
  ✓ aware CLI installed
  ✓ 68 AECO agents available via `aware agent install`
  ✓ aware-aeco plugin registered with: claude-code, codex

$ claude-code
  ✓ plugin: aware-aeco · 68 agents available
...
- Apache 2.0 core CLI
- <!--stat:agents_total-->68<!--/stat--> first-party agents across engineering · architecture · construction · visualization · cross-cutting (`tekla`, `revit`, `rhino`, `autocad`, `trimble-connect`, `procore`, `navisworks`, `idea-statica-26`, `microsoft-365`, `aware-agent-builder`, …) — full list in [`registry-index.json`](../registry-index.json)
