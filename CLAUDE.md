...
- **That route covers plain `X.Y.Z` only.** `tag.yml`'s input guard rejects a prerelease or build suffix — deliberately, since the value reaches `GITHUB_ENV` in a job holding a write-capable token — while `--bump` accepts one and `release.yml` ships a hyphenated tag as a prerelease. So a prerelease tag has no dispatch route and must be pushed from a checkout with tag-push rights. `--bump` says which of the two applies to the version you passed.
...
- <!--stat:agents_total-->79<!--/stat--> agents under `20-agents/` — <!--stat:agents_curated-->31<!--/stat--> curated (tekla, trimble-connect, navisworks, microsoft-365, google-workspace, …) + <!--stat:agents_reflected-->48<!--/stat--> reflected (revit, rhino, autocad, idea-statica, …); full list in `registry-index.json`
